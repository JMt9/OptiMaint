...
class BearingOverhangConsumer:
    # =========================
    # CONFIG (EDITA AQUI)
    # =========================
    BOOTSTRAP_SERVERS = "127.0.0.1:9092"
    GROUP_ID = "bearing-consumer-overhang"
    AUTO_OFFSET_RESET = "earliest"

    TOPIC_IN = "bearing_overhang_features"

    # Modelo
    OVERHANG_MODEL_BUNDLE_PATH = "models/overhang/overhang_model.pkl"

    # Features usadas por el submodelo (CSV con columna 'feature')
    RF_FEATURES_CSV = r"C:\Users\Usuario\Desktop\master\1er_semestre\parte2\reto02\models\overhang\features_info\rf_selected_features_for_gridsearch.csv"
    ...
    def __init__(self) -> None:
        ...
        self.bundle = joblib.load(self.OVERHANG_MODEL_BUNDLE_PATH)
        if not isinstance(self.bundle, dict) or "model" not in self.bundle:
            raise ValueError("overhang_model.pkl debe ser dict con al menos key: 'model'.")
        ...
        # (Opcional) check con subclass_order del bundle
        ...
        # Guardar CSV completo de entrada overhang
        self.full_csv_path = os.path.join(self.DATALAKE_ROOT, "fallo", "overhang", "overhang.csv")
        self.full_ids = self._load_existing_ids(self.full_csv_path)  # dedupe por sample_id
        self.buf_full_rows: List[Dict[str, Any]] = []

        # Dedupe sets y buffers por subtipo (ids+path)
        self.id_sets = {}
        self.buf_rows = {}
        for sub in self.SUBLABELS:
            p = self._path_ids_csv(sub)
            self.id_sets[sub] = self._load_existing_ids(p)
            self.buf_rows[sub] = []  # list[dict]

    # ---------- dirs/paths ----------
    def _ensure_dirs(self) -> None:
        base = os.path.join(self.DATALAKE_ROOT, "fallo", "overhang")
        os.makedirs(base, exist_ok=True)
        for sub in self.SUBLABELS:
            os.makedirs(os.path.join(base, sub), exist_ok=True)

    def _path_ids_csv(self, sub: str) -> str:
        return os.path.join(self.DATALAKE_ROOT, "fallo", "overhang", sub, f"{sub}.csv")
    ...
    def _prepare_X(self, record: Dict[str, Any], sample_id: str) -> pd.DataFrame:
        ...
        if missing:
            ...
            if self.LOG_MISSING_FEATURES_EVERY_N > 0 and (self._missing_feat_counter % self.LOG_MISSING_FEATURES_EVERY_N == 0):
                ...
                print(f"[WARN] sample_id={sample_id} faltan {len(missing)} features (OVERHANG). Ejemplo: {shown}")
            if self.STRICT_REQUIRE_ALL_FEATURES:
                self._skipped_missing += 1
                raise KeyError(f"Faltan {len(missing)} features (OVERHANG) para sample_id={sample_id}")
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                # patrón clase: si None, termina
                if msg is None:
                    print("[CONSUMER-OVERHANG] msg is None -> terminando.")
                    break

                if msg.error():
                    print(f"[CONSUMER-OVERHANG-ERROR] {msg.error()}")
                    continue
                ...
                # Guardar CSV completo (1 vez por sample_id)
                ...
                try:
                    ...
                    print(f"[SKIP] sample_id={sample_id} no se pudo preparar X (OVERHANG): {e}")
                    continue

                try:
                    y_pred = self.model.predict(X)[0]
                except Exception as e:
                    print(f"[PREDICT-ERROR] (OVERHANG) sample_id={sample_id} error={e}")
                    continue
                ...
                if self.LOG_EVERY_N > 0 and (self.processed % self.LOG_EVERY_N == 0):
                    dt = time.time() - t0
                    print(
                        f"[OVERHANG] processed={self.processed} "
