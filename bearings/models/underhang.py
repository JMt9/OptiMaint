...
# CONFIGURACION
CSV_PATH = "data/features_underhang.csv"
...
SUBCLASS_ORDER = ["ball_fault", "cage_fault", "outer_race"]

# K features a probar en ablación
TOP_K_LIST = [1, 10, 20, 30, 40, 50, 75, 100, 125]
...
def main():
    ...
    if MAIN_LABEL_COL in df.columns:
        ...
        ok = s.isin(["4", "underhang"]).all()
        if not ok:
            raise ValueError("Este CSV debería venir filtrado SOLO a UNDERHANG (fault='underhang' o 4).")
    ...
    out_model = os.path.join(OUT_DIR, "underhang_model.pkl")
