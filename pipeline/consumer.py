...
class BearingLevel1Consumer:
    ...
    def __init__(self) -> None:
        ...
        # Aviso si bundle trae otra lista distinta (no rompe)
        ...
        self.id_sets = {
            ...
            # ahora over/under también son MIN (id+path) en su CSV
        ...
        # -------- Buffers --------
        # filas min: sample_id + file_path para TODAS las salidas en disco incl. over/under
        self.buf_rows_min = {k: [] for k in self.id_sets.keys()}  # list[dict]
        # buffers no necesarios en disco para over/under full (ya no se guardan)
        self._ensure_dirs()
    ...
    # =========================
    # Prepare X (SOLO features, SIN imputación)
    # =========================
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                # patrón clase: si no hay msg, termina
                if msg is None:
